...
usersList.show= async ({filter,show,search,page})=>{
    ...
    const sortBy = ()=>{ 
        switch (filter) {
            case 'Recently Login':return { lastLogin:-1 }
            case 'Recently Registered':return { createdAt:-1 }
            case 'Verified':return { isVerified:-1 }
    ...
    const DatabaseQuery= async ()=>{
        ...
        if(filter === 'Onlines'){
            const onlineUsersId = await redis.smembers('online:users')
    ...
    .then(async users=>{
        if(!users){ return Promise.reject('no user has been registered yet')}
        ...
        return {
            Users, totalUsers, hasNextPage, hasPreviousPage, lastPage, show, filter, search,
            currentPage:page,errorMessage:null
        };
    }).catch(e=>{
        return {
            Users:[],
