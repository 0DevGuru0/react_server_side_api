...
router.get("/auth/google", requireNotLoggedIn, googleAuth);
router.get(
...
router.get("/logout", requireLogin, rootCtr.logOut);
router.get("/current_user", requireLogin, (req, res) => {
	res.send(req.user);
});
router.get("/emailverify", rootCtr.emailVerification, rootCtr.redirectToRoot);
router.get("/resetPassword", rootCtr.resetPassword);
